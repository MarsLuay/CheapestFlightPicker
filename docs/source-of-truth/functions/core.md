...
| `repriceOption` / `repriceOneWayOption` | `search.ts` | private | Fresh exact with `bypassCache` | Provider I/O |
...
| `combineBookingSources` / `prefersDirectBooking` / `isLikelyDirectAirlineBookingOption` / `clampTimeWindow` / `combineTwoOneWays` / `findCheapest*` / `mapWithConcurrency` / Google enum maps | `utils.ts` | various | Shared helpers | Pure / concurrent async |
