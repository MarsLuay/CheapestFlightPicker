...
| `App` | `App.tsx` | default component | Form + search orchestration, including the persisted equal-price mileage preference | localStorage; abort; hosted gate |
...
| `load/saveSavedSearchPreferences` / `load/saveSavedSearchDates` / `load/saveSavedOrigin` | prefs libs | Browser persistence; search preferences default missing `prioritizeMileFlights` to false |
