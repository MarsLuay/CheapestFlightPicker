...
**Invariants:** Hosted mode blocks search; every `runFlightSearch` forces `maxResults = maxSearchResults`; the persisted “Prioritize mile flights” switch affects equal-price ties only; admin is `` ` ``-gated Advanced surface; admin requests use the runtime `window.__ADMIN_TOKEN__` value as an `x-admin-key` header when configured
