...
**Invariants:** Hosted mode blocks search; every `runFlightSearch` forces `maxResults = maxSearchResults`; the persisted “Prioritize mile flights” switch affects equal-price ties only; admin is `` ` ``-gated Advanced surface
